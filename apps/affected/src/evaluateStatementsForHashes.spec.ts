...
describe('evaluate-statements-for-hashes.spec', () => {
  ...
  it('should return the correct hash for markdown files', async () => {
    const files = [
      "100644 a 0	docs/readme.md",
      "100644 b 0	src/index.js",
      "100644 c 0	src/guide.md"
    ];
  ...
  it('should return the correct hash for dependency of files', async () => {
    ...
    }).filter(f => (f.file.startsWith('apps/affected/') || f.file.startsWith('dist/apps/affected/')) && !f.file.endsWith('.md')).sort();
    const hash = crypto.createHash('sha1')
                       .update(matchedFiles.map(x => x.hash).join('\n') + '\n')
                       .digest('hex');
